"""This script will perform Fast cost volume with correlation."""
...
def readcolorppm(filename):
    ...
    f = open(filename)
    f.readline().splitlines()  # number of colors
    size_x, size_y = f.readline().split()
    f.readline().splitlines()  # max value
    data = f.read().split()
    data = map(int, data)
...
def save_pfm(file, image, scale=1):
    """Save the specified images / disparity map as a PFM file."""
    color = None
...
if __name__ == '__main__' or True:
    ...
    costfname = 'data/fcv_corr_cost_'+image+'_r'+str(r)+'_cr'+str(cr)+'.npy'
    costfname1 = 'data/fcv_corr_cost1_'+image+'_r'+str(r)+'_cr'+str(cr)+'.npy'
    ...
    if cost == 1:
        ...
        for y in range(cr, m-cr):
            for x in range(cr, n-cr):
                ...
                c_color[y, x, maxDisp-d-1:] = cv2.matchTemplate(
                    Rimg[y-cr:y+cr+1, x-cr-d:x+cr+1],
                    Limg[y-cr:y+cr+1, x-cr:x+cr+1], cv2.TM_CCORR_NORMED)[0]
                c1_color[y, x, maxDisp-d-1:] = cv2.matchTemplate(
                    Limg_1[y-cr:y+cr+1, x-cr-d:x+cr+1],
                    Rimg_1[y-cr:y+cr+1, x-cr:x+cr+1], cv2.TM_CCORR_NORMED)[0]
        ...
        np.save(costfname,c_color)
        np.save(costfname1,c1_color)
        ...
        for y in range(m):
            for x in range(n):
                if np.abs(labels_left1[y][x] -
                          labels_right1[y][x - labels_left1[y][x]]) >= lim:
                    final_labels1[y][x] = -1

        plt.figure()
        plt.title('Final labels before GIF')
        plt.imshow(final_labels1, cmap=plt.cm.gray)
        fstr = 'data/res/fcv_corr/'+image+'_fcv_corr_noGIF_cr'+str(cr)+'_r'+str(r)+'.jpg'
        plt.imsave(fstr, final_labels1, cmap=plt.cm.gray)
    else:
        c_color = np.load(costfname)
        c1_color = np.load(costfname1)
    ...
    if filtering == 1:
        ...
        fstr = 'data/res/fcv_corr/'+image+'_fcv_corr_cr'+str(cr)+'_r'+str(r)+'.jpg'
        plt.imsave(fstr, final_labels, cmap=plt.cm.gray)

        if image == 'mot':
            fstr = 'data/res/fcv_corr'+image+'_fcv_corr_cr'+str(cr)+'_r'+str(r)+'.pfm'
            file = open(fstr, 'wb')
