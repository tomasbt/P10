...
if __name__ == '__main__' or True:
    ...
    for d in range(maxDisp):
        dispvol2[d,:,:] = myGIF(LimgG,dispVol[:,:,d],r,eps)
        dispvol2_1[d,:,:] = myGIF(RimgG,dispVol1[:,:,d],r,eps)
    ...
    for y in range(m):
        for x in range(n):
            if np.abs(dispmap_l[y][x]-dispmap_r[y][x-dispmap_l[y][x]])>=lim:
                dispmap_final[y][x] = -1
    ...
    plt.figure()
    fstr = 'data/res/'+image+'_fcv_gray.png'
