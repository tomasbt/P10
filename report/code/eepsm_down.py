...
def myy_pyrDown(img):
    '''
    Using gaussian blur on vertical axis to perform pyrDown
    '''
    tmp = cv2.GaussianBlur(img, (1, 5), 0)
    tmp = tmp[::2, :]
    return tmp
...
def myy_pyrUp(img):
    '''
    Using gaussian blur on vertical axis to perform pyrUp
    '''
    tmp = np.zeros((img.shape[0] * 2, img.shape[1]))
    tmp[::2, :] = img
    gknl = cv2.getGaussianKernel(5, 1) * 2
    tmp = cv2.filter2D(tmp, -1, gknl)
...
def pyrUp_int(img):
    '''
    Using interpolation to resize to disparity map to the original size
    '''
    tmp = cv2.resize(img, (0, 0), fx=1, fy=2, interpolation=cv2.INTER_LINEAR)
...
if __name__ == "main" or True:
    ...
    iplvl = 2  # Number of levels used in image pyramid
    maxDisp = fdict[image][2]  # / (2**iplvl)  # maximum disparity value in the
    # scene
    ...
    start = time.time()
    for lvl in range(1, iplvl + 1):
        Limg = myy_pyrDown(Limg)
        Rimg = myy_pyrDown(Rimg)

    Limg = Limg[:, :, ::-1].astype(np.int64)
    Rimg = Rimg[:, :, ::-1].astype(np.int64)
    ...
    aggre_T[Limg.shape[0] - 1, :, d] = horz_aggre[Limg.shape[0] - 1, :, d]
    ...
    if consistancy_check:
        ...
        # Left scan value calculation
        aggre_L[:, Rimg_m.shape[1] - 1, :] = costMat_m[:, Limg.shape[1] - 1, :]
        for x in range(Limg.shape[1] - 2, 0, -1):
            for d in range(maxDisp):
        ...
        # top scan value calculation
        aggre_T[Rimg.shape[0] - 1, :, d] = horz_aggre[Limg.shape[0] - 1, :, d]
        for y in range(Limg.shape[0] - 2, 0, -1):
            for d in range(maxDisp):
                aggre_T[y, :, d] = horz_aggre[y, :, d] + perme_t[y + 1, :] * \
                    aggre_T[y + 1, :, d]
    ...
    dispmap_final_filled = dispmap_final_filled.astype(np.uint16)
    for lvl in range(1, iplvl + 1):
        dispmap_final_filled = myy_pyrUp(dispmap_final_filled)

    print 'It took', time.time() - start
    # limit max values
    high_val_indices = dispmap_final_filled > maxDisp + 10
    dispmap_final_filled[high_val_indices] = maxDisp + 10

    # # save the out as .png for the report
    plt.figure()
    fstr = 'data/res/' + image + '_cr' + str(cr) + '_s' + str(sig) + '_al' + \
        str(al) + '_eepsm_down.png'
    ...
    if any(image in s for s in imgList):
        fstr = 'data/res/' + image + '_eepsm_down.pfm'
        file = open(fstr, 'wb')
